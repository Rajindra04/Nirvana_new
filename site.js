/* ============================================================
   NIRVANA BIOTECH — COMMERCIAL SITE shared script
   Loads data.json, hydrates per-page content, handles nav + modal.
   Same admin engine as the research site, separate Worker/domain.
   ============================================================ */
...
/* ---------------- admin config ---------------- */
// This MUST point at a separate Worker deployment from the research site —
// different repo, different GITHUB_OWNER/REPO, different ADMIN_PASSWORD.
// Placeholder until you deploy: copy worker.js + wrangler.toml from this
// package into their own folder and deploy under a new Worker name, e.g.
// "nirvana-commercial-admin" (see WRANGLER_GUIDE.md).
const ADMIN_API_BASE = "https://nirvana-commercial-admin.rajindra04.workers.dev";
const SESSION_KEY = "nbc_admin_session";
const PENDING_PATCH_KEY = "nbc_pending_patch";   // unsaved field edits, deep-merged onto data.json
const PENDING_IMAGES_KEY = "nbc_pending_images"; // unsaved image uploads, base64, keyed by repo path
  ...
  overlay.innerHTML = `
    <div class="modal-box" style="max-width:560px;">
      <div class="modal-body">
        ...
        <input id="${uid}-linklabel" type="text" placeholder="e.g. View datasheet">
...
/* ---- add / remove array items (products, services, about highlights) ---- */
const ARRAY_TEMPLATES = {
  products: { name: 'New Product', sku: 'NB-000', desc: 'Description goes here.', specs: [], imageUrl: '', fullText: '', linkUrl: '', linkLabel: '' },
  services: { title: 'New Service', icon: '01', desc: 'Description goes here.' },
  highlights: { title: 'New Highlight', desc: 'Description goes here.' },
};
// highlights lives nested under "about"
const NESTED_ARRAY_PARENT = { highlights: 'about' };
...
/* ---------------- spec strip (signature motif) ---------------- */
// Renders a row of key:value pairs in mono type, e.g. SENS: 98.2% · SPEC: 99.1%
// Pass an array of {label, value} via data-spec-items (JSON) on the element,
// or call renderSpecStrip(el, items) directly from a page's renderPage().
function renderSpecStrip(el, items) {
  if (!el) return;
  if (!items || !items.length) { el.innerHTML = ''; return; }
  el.innerHTML = items.map(it => `
    <span class="spec-item">${escapeHtml(it.label)}: <b>${escapeHtml(it.value)}</b></span>
  `).join('');
}
function injectTraces() {
  // No-op on the commercial site — kept as a name so shared init code
  // (DOMContentLoaded below) doesn't need a separate branch. Spec strips are
  // rendered per-product inside each page's renderPage() via renderSpecStrip().
}
...
function applySectionBackground(elId, url) {
  ...
  if (!el) return;
  if (url && url.trim()) {
    // Light-mode hero: a soft white-to-paper scrim keeps dark text legible
    // over a photo, instead of the research site's dark scrim (which assumes
    // light text on a dark background).
    el.style.backgroundImage = `linear-gradient(rgba(250,250,247,0.88), rgba(250,250,247,0.94)), url('${url}')`;
